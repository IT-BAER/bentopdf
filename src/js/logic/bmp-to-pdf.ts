...
export async function bmpToPdf() {
  if (state.files.length === 0) {
    showAlert(getTranslations().bmpToPdf.noFilesTitle, getTranslations().bmpToPdf.noFilesMessage);
    return;
  }
  showLoader(getTranslations().bmpToPdf.converting);
  try {
    ...
    showAlert(
      ...
      getTranslations().bmpToPdf.error
    );
  } finally {
    hideLoader();
  }
