...
async function setupFileInput(inputId: any, docKey: any, displayId: any) {
  ...
  const handleFile = async (file: any) => {
    if (!file || file.type !== 'application/pdf')
      return showAlert(getTranslations().comparePDFs.invalidFile, getTranslations().comparePDFs.selectValidFile);
    ...
    try {
      showLoader(getTranslations().comparePDFs.loadingFile.replace('{file}', file.name));
      ...
      showAlert(
        ...
        getTranslations().comparePDFs.couldNotLoad
      );
      console.error(e);
    } finally {
      hideLoader();
