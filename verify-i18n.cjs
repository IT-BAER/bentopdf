...
for (const file of files) {
  ...
  const fileIssues = [];
  const usesFooterPartial = content.includes('{{> footer }}');

  // Check for navigation i18n
  if (
    content.includes('href="/about.html"') &&
    !content.includes('data-i18n="nav.about"')
  ) {
    fileIssues.push('Missing nav.about i18n');
  }
  if (
    content.includes('href="/contact.html"') &&
    !content.includes('data-i18n="nav.contact"')
  ) {
    fileIssues.push('Missing nav.contact i18n');
  }

  // Check for footer i18n only on pages with inline footer markup.
  // Pages using the shared footer partial inherit i18n centrally.
  if (!usesFooterPartial) {
    if (
      content.includes('>Company<') &&
      !content.includes('data-i18n="footer.company"')
    ) {
      fileIssues.push('Missing footer.company i18n');
    }
    if (
      content.includes('>Legal<') &&
      !content.includes('data-i18n="footer.legal"')
    ) {
      fileIssues.push('Missing footer.legal i18n');
    }
    if (
      content.includes('>Follow Us<') &&
      !content.includes('data-i18n="footer.followUs"')
    ) {
  ...
  if (
    content.includes('Click to select') &&
    !content.includes('data-i18n="upload.clickToSelect"')
  ) {
    fileIssues.push('Missing upload.clickToSelect i18n');
  }

  // Check footer links only on pages with inline footer markup.
  if (!usesFooterPartial) {
    if (
      content.includes('>About Us<') &&
      !content.includes('data-i18n="footer.aboutUs"')
    ) {
      fileIssues.push('Missing footer.aboutUs i18n');
    }
    if (
      content.includes('>FAQ<') &&
      !content.includes('data-i18n="footer.faq"')
    ) {
      fileIssues.push('Missing footer.faq i18n');
    }
    if (
      content.includes('>Contact<') &&
      !content.includes('data-i18n="footer.contact"')
    ) {
      fileIssues.push('Missing footer.contact i18n');
    }
    if (
      content.includes('>Licensing<') &&
      !content.includes('data-i18n="footer.licensing"')
    ) {
      fileIssues.push('Missing footer.licensing i18n');
    }
    if (
      content.includes('>Terms of Use<') &&
      !content.includes('data-i18n="footer.terms"')
    ) {
      fileIssues.push('Missing footer.terms i18n');
    }
    if (
      content.includes('>Privacy Policy<') &&
      !content.includes('data-i18n="footer.privacy"')
    ) {
...
for (const key of enKeys) {
  ...
  const deTool = deTools[key];
  const needsNameSubtitle =
    enTool &&
    typeof enTool === 'object' &&
    !Array.isArray(enTool) &&
    (Object.prototype.hasOwnProperty.call(enTool, 'name') ||
      Object.prototype.hasOwnProperty.call(enTool, 'subtitle'));

  if (!needsNameSubtitle || !deTool || typeof deTool !== 'object') {
    continue;
  }
